...
#(2). 최근 업데이트 일자 및 현재 일자확인
query = """
    SELECT * FROM date
"""
cur.execute(query)
startDate = cur.fetchone() #최근 업데이트 일자

now = time.localtime()
endDate = time.strftime('%Y%m%d', now) #현재 일자
...
#(1). 데이터프레임 정리
dat_final = pd.concat(dataframe) #데이터가 없으면 여기서 오류남!
...
#%%7. DB에 데이터 적재
con_data_current = pymysql.connect(host="localhost", user="root", password="0000", db="data_current", charset='utf8')
cur_data_current = con_data_current.cursor()

con_infor = pymysql.connect(host="localhost", user="root", password="0000", db="infor", charset='utf8')
cur_infor = con_infor.cursor()
...
for i in range(dat_final.shape[0]):
    ...
    try:
        ...
        VALUES {}
        """.format(target)
        cur_data_current.execute(query)
        con_data_current.commit()
    except Exception as e:
        print(f"Error inserting data into 전용면적: {str(e)}")
        con_data_current.rollback()
...
for i in range(dat_error.shape[0]):
    ...
    try:
        query = """
            INSERT INTO 전용면적_에러(update_date, sigunguCd, bjdongCd, error_name, error_num) VALUES {}
        """.format(target)
        cur_data_current.execute(query)
        con_data_current.commit()
    except Exception as e:
        print(f"Error inserting data into 전용면적_에러: {str(e)}")
        con_data_current.rollback()
...
try:
    ...
    """.format(update_time)
    cur_infor.execute(query)
    con_infor.commit()
except Exception as e:
    print(f"Error updating last update date: {str(e)}")
    con_infor.rollback()

con_data_current.close()
con_infor.close()